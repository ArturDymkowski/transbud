<?php

return [
    'general' => [
        'close' => 'Close',
        'save' => 'Save',
        'today' => 'Today',
        'extra_info' => 'Extra Info',
        'basic_info' => 'Basic information',
        'not_selected' => '-- not selected --',
        'saved_success' => 'Saved successfully',
        'updated_success' => 'Record updated',
        'deleted_success' => 'Successfully deleted',
        'deleted_success_count' => 'Deleted :count records',
        'restored_success' => 'Record restored',
        'force_deleted_success' => 'Record permanently deleted',
        'force_delete_blocked' => 'Cannot permanently delete — this record is still in use elsewhere.',
        'delete_blocked_active_delivery' => 'Cannot delete — this record is linked to an active delivery (assigned or in progress).',
        'download_document' => 'Download document',
        'delete_document' => 'Delete document',
        'from' => 'From',
        'to' => 'To',
        'learn_more' => 'Learn more',
        'no_preview' => 'No preview',
        'document_preview' => 'Document preview',
    ],

    'nav' => [
        'toggle_sidebar' => 'Toggle Sidebar',
        'toggle_mobile_menu' => 'Toggle Mobile Menu',
        'edit_profile' => 'Edit profile',
        'account_settings' => 'Account settings',
        'support' => 'Support',
        'sign_out' => 'Sign out',
    ],

    'sidebar_widget' => [
        'title' => '#1 Tailwind CSS Dashboard',
        'description' => 'Leading Tailwind CSS Admin Template with 500+ UI Component and Pages.',
        'purchase_plan' => 'Purchase Plan',
    ],

    'languages' => [
        'pl' => 'Polish',
        'en' => 'English',
    ],

    'pagination' => [
        'previous' => 'Previous',
        'next' => 'Next',
        'page' => 'Page',
        'showing_from' => 'Showing',
        'showing_to' => 'to',
        'showing_of' => 'of',
        'showing_total' => 'total',
        'no_results' => 'No results',
    ],

    'tables' => [
        'per_page' => 'Per page',
        'active' => 'Active',
        'active_plural' => 'Active',
        'inactive_plural' => 'Inactive',
        'all' => 'All',
        'yes' => 'Yes',
        'no' => 'No',
        'country' => 'Country',
        'search' => 'Search',
        'search_placeholder' => 'Search...',
        'filters' => 'Filters',
        'status' => 'Status',
        'selected_records' => 'Selected records',
        'unselect_all' => 'Unselect all',
        'delete_selected' => 'Delete selected',
        'confirm_delete_selected' => 'Are you sure you want to delete the selected records?',
        'clear_all' => 'Clear all',
        'actions' => 'Actions',
        'show' => 'Preview',
        'edit' => 'Edit',
        'delete' => 'Delete',
        'restore' => 'Restore',
        'confirm_restore' => 'Are you sure you want to restore this record?',
        'force_delete' => 'Delete permanently',
        'confirm_force_delete' => 'Are you sure you want to PERMANENTLY delete this record? This cannot be undone — the data will be lost irreversibly.',
        'list' => 'List',
        'no_results' => 'No results found',
        'create' => 'Create',
        'trashed' => 'Deleted records',
        'without_trashed' => 'Without deleted records',
        'with_trashed' => 'Include deleted records',
        'only_trashed' => 'Only deleted records',
    ],

    'dashboard' => [
        'title' => 'Dashboard',
        'empty_title' => 'Nothing to show yet',
        'empty_description' => 'Soon you\'ll see alerts here about drivers\' documents nearing their expiry date (driving license, ID card).',
        'expiring_documents' => 'Expiring documents',
        'box_30_days' => 'In 30 days',
        'box_14_days' => 'In 2 weeks',
        'box_7_days' => 'In a week',
        'empty_box' => 'No upcoming deadlines',
        'days_left' => ':days days left',
        'day_left' => '1 day left',
        'expires_today' => 'Expires today',
        'expired_days_ago' => 'Overdue by :days days',
        'expired_one_day_ago' => 'Overdue by 1 day',
    ],

    'menu' => [
        'access' => 'Access',
        'dictionaries' => 'Dictionaries',
    ],

    'address' => [
        'address' => 'Address',
        'zipcode' => 'Zipcode',
        'city' => 'City',
        'street' => 'Street',
        'house_nr' => 'House nr',
        'apartment_nr' => 'Apartment nt',
        'country' => 'Country',
    ],
];
