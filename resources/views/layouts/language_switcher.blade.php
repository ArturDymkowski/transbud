            ...
            @foreach(config('app.available_locales') as $locale)
                <a href="{{ route('language.switch', $locale) }}"
                   class="flex items-center gap-3 px-4 py-3 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
                    <span class="font-semibold tracking-widest text-brand-500"> {{ Str::upper($locale) }} </span>
                    <span class="text-gray-800 dark:text-white/90"> {{ __('labels.languages.' . $locale) }} </span>
                </a>
            @endforeach
        </div>
    </div>
